...
This can be done automatically in the bat-file. Do not use this code unless necessary, if something goes wrong, you may have changed the code lage unintentionally. 
```
...
Give the main script a short, descriptive name. The name of the main script is included in the bat-file command. To avoid having to edit the bat-file, you should chose a name that you will not change. The main script for generating an outbreak report is by convention named batch_outbreak.R.

The main script sources one or more R-scripts that together perform the tasks to generate a product, for example a report. The batch script will usually consist of three parts:
...
### Set up the R-environment
In this part you want to set up the R-environment for all script that are called (sourced) by the main script. That includes attaching all packages needed, setting up global variables and functions, and loading support data.
...
We advise to have one section in which the global variables are set. In this example there are two global variables, the path to the R-scripts and today's date. The path to the R-script cannot be set automatically by package 'here' at the server. 
```{r, include = TRUE}
## Paths and filenames ----
...
### Mark the end of the main script
At the end of the script it is useful to print a message that marks the end of the script. That is because you in the next step error messages and warnings will be checked and messages after the end-mark don't need to be checked. When finishing a batch job there will often be error messages due to lack of writing access. To avoid these to turn up in the check, see next section, you mark the end of the script. There is no need for this step unless you run the check log step. 
```{r, include = TRUE} 
print("Ferdig_batch_outbreak")
``` 
...
There is need for a rutine that delete old log files from the log archive as there aren't any use for more than the most recent log files. This rutine hasn't been developed yet and old log files must be deleted manually. 

### Set up the R-environment
To set up the R-environment is done comparable to the set up the R-environment for the main script. An example is given below. 

#### Attach packages
The packages given is the one that you will need for this script. 
```{r, include = TRUE} 
library(NVIbatch) 
...
### Save the log in the archive
The Rout-file is saved with a new name that includes today's date before the file extension. By convention, the file is saved in the directory "batchlogg". 
```{r, include = TRUE} 
# Save the Rout-file ----
...
The function gather_messages is used to father errors and warnings from the log-file. 
```{r, include = TRUE} 
...
# SEND EMAIL ----
sendmail(from = "<common_email@domain.no>",
