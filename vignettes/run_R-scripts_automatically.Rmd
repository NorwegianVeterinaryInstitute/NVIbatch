...
Give the main script a short, descriptive name. The name of the main script is included in the bat-file command so try to chosen a name that you will not change to avoid having to edit the bat-file. The main script for gererating an outbreak report is by convention named batch_outbreak.R

The main script sources different R-scripts that perform the tasks. The batch script will usually consist of three parts:
...
### Set up the R-environment
In this part you want to set up the R-environment for all script that are called by the main script. That includes attaching all the needed packages, setting up global variables and functions, and load support data.

#### Attach packages
You may use use_pkg for packages at Cran and use_NVIverse for packages within NVIverse. These functions accept a vector with packages and will attach the packages if installed and install the package first if not installed. To install packages, the user running the batch needs writing rights.
```{r, include = TRUE}
...
#### Global variables
It is useful to have one place where all global variables are set. In this example there are two global variables, the path to the R-scripts and today's date. The path to the Rscript cannot be set automatically by package 'here' at the server. 
therefore the most robust is to set the path name manually. 
```{r, include = TRUE}
## Paths and filenames ----
# The path to the R-scripts.
rscripts <- paste0(set_dir_NVI("Hendelser"),"xxxx/xxxx/")
rscripts <- dirname(rstudioapi::getSourceEditorContext()$path)
...
#### Import support data
Usually you may be dependent on loading some registers that are used later. Until we have access to databases with this registers, these can be imported in this part of the script. 
```{r, include = TRUE} 
## Import support data ----
...
### Source the R-scripts 
These scripts are performing thee tasks that shall end in the report(s). Here an example with some of the scripts that usually will be part of preparing an outbreak report. For simplicity is the names of the scripts in the outbreak report standardized. 
```{r, include = TRUE} 
# OUTBREAK REPORT ----
