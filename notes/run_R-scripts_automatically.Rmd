...
## Overwiev
To set up R-scripts to run automatically at scheduled times is done with three processes.

1. Set up Task Scheduler on Windows  to run a bat-script automatically.
2. Write a the bat-script that call R and run the R-scripts. We call two R-scripts, one that sources all R-scripts to do the job and one that check the log and report the results.
3. Write the R-scripts to do the job
